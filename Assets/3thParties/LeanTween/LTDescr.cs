...
public class LTDescr
{
    ...
    public LTDescr setImageFill(UnityEngine.UI.Image image)
    {
        this.uiImage = image;
        this.type = TweenAction.IMAGE_FILL;
        this.easeType = LeanTweenType.linear;
        this.initInternal = () => { this.fromInternal.x = trans.GetComponent<UnityEngine.UI.Image>().fillAmount; };
        this.easeInternal = () => { this.trans.GetComponent<UnityEngine.UI.Image>().fillAmount = easeLinear().x; };
        return this;
    }
	...
	* LeanTween.moveLocalZ( gameObject, 10f, 1f).setSpeed(0.2f) // the given time is ignored when speed is set<br>
	*/
    public LTDescr setImageFill(float imageFill)
    {
        this.speed = speed;
        if (this.hasInitiliazed)
            initSpeed();
